...
class SMLTopo(Topo):
    ...
    def build(self):
        ...
        for i in range(NUM_WORKERS):
            # set ip explicitly for safety
            host = self.addHost(f"w{i}", ip = f"10.0.0.{i+1}")
    ...
    for key, value in switch.ports.items():
        if key.name.startswith(switch.name):
            ...
            switch.insertTableEntry(
                ...
                match_fields={"hdr.ipv4.dstAddr": [str(ip_address(port_to_node[value].IP())), 32]},
                action_name="TheIngress.ipv4_forward",
                action_params={
                    "dstAddr": str(port_to_node[value].MAC()),
                    "port": value
                }
            )
